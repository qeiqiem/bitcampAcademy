	...
	<div class="body_container">
		<div class="map_container">
			...
			<div class="custom_typecontrol radius_border">
				<span id="btnRoadmap" class="selected_btn_map"
					onclick="setMapType('roadmap')">지도</span> <span id="btnSkyview"
					class="btn_sky" onclick="setMapType('skyview')">스카이뷰</span>
			</div>
			...
			<div class="slide_container">
				<div class="slide">
					<div class="slide_top">
						...
						<span class="checkbox checkbox-circle">
							<input class="findPoint" type="checkbox">
						...
						<div class="slide_search">
							<form onsubmit="searchPlaces(); return false;">
								<input class="input_search" type="text" id="keyword"
					...
					<div class="footer single">						
						...
						<div>
							<button class="infoBtn checked" id="infoData">업체정보</button>
							...
							<div class="cardinfo">
								<table id="single_table">
									...
									<tr>
										<td><p id="s_phone"
												style="color: rgb(90, 90, 90); margin: 0"></p></td>
									</tr>
									<tr>
										<td><p id="s_address"
												style="color: rgb(90, 90, 90); margin: 0"></p></td>
							...
							<div class="cominfo">
								<div class="comhead">
									<label for="latest"><input type="radio" class="latest"
										name="review" checked="checked">최신순</label> <label for="grade"><input
										type="radio" class="grade" name="review">평점순</label>
									<p class="allCom"></p>
								</div>
				...
				<div class="slide_res">
					<p id="res_p">
						&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp&nbsp<i class="far fa-edit"></i>&nbsp&nbsp예약하기
					</p>
					<div class="rescont">
						<p
							style="width: 94%; height: 35px; padding-block-start: 20px; border-bottom: 1px solid lightgray;">주문신청</p>
						<table id="resShortOpt"></table>
					</div>
					<div class="userInfo">
						<p style="font-weight: bolder;">
							예약자 &nbsp;<span style="font-weight: 500;">${user.name}</span>
						</p>
						<p style="font-weight: bolder;">
							연락처 &nbsp;<span style="font-weight: 500;">${user.phone}</span>
						</p>
						<p style="font-weight: bolder;">
							이메일 &nbsp;<span style="font-weight: 500;">${user.email}</span>
						</p>
					</div>
					<div class="sellerInfo">
						...
						<div id="agreement">
							...
							<div class="termsText">
								<span>1. 개인정보를 제공받는 자 : <span class="bname"></span> <br>
									<br> 2. 제공하는 기본 개인정보 항목: [필수] kkaekkt 아이디, 이름, (휴대)전화번호,
									주소<br> ※ 추가 제공하는 필수항목<br> - 배송, 방문 등이 필요한 상품 구매 시 :
									주소<br> - 해외 여행 관련 상품 구매 시 : 여권상 영문명, 여권번호 끝 4자리, 생년월일,
									이메일주소, 카카오톡ID, 동행 아동정보(여권상 영문명, 생년월일, 신장)<br> - 병원을 이용하는
									경우: 생년월일<br> <br> 3. 개인정보를 제공받는 자의 이용목적 : 사업자회원과
									예약이용자의 원활한 거래 진행, 서비스 분석과 통계에 따른 혜택 및 맞춤 서비스 제공, 민원처리 등 고객상담,
									고객관리, 서비스 이용에 따른 설문조사 및 혜택 제공, 분쟁조정을 위한 기록보존<br> <br>
									4. 개인정보를 제공받는 자의 개인정보 보유 및 이용기간 : kkaekkt 회원탈퇴 시 또는 위 개인정보 이용목적
									달성 시 까지 이용합니다.<br> <br> 5. 동의 거부권 등에 대한 고지 : 정보주체는
									개인정보 제공 동의를 거부할 권리가 있으나, 이 경우 상품 및 서비스 예약이 제한될 수 있습니다.<br>
									<br>
							</div>
						</div>
						<ul>
						...
						<p
							style="width: 92%; font-size: 14px; color: var(- -text-gray); padding-bottom: 45px; background-color: white;">예약
							서비스 이용을 위한 개인정보 수집 및 제3자 제공, 취소/환불 규정을 확인하였으며 이에동의합니다.</p>
					</div>
					<div class="comBtnDiv">
...
<script>
	...
	/* 지도 api에서 제공하는 이벤트 */
	var markers = [];
	var mapContainer = map = document.getElementById('map'), mapOption = {
	...
	//키워드검색,데이터바인딩 나중에 분기로치고 ajax 한번만돌게 수정해야함
	...
	function bindinglandry(keyaddr) {
		...
		var keyaddr = keyaddr
		var arrkey = keyaddr.split(",");
		/* keyaddr = arrkey[0]+arrkey[1] */
		keyaddr = arrkey[0]
		let apiSearch = arrkey[0] + arrkey[1]
		...
		$.ajax({
			...
			success : function(data) { //성공후 처리는 추후 진행.                         
				...
				// 장소검색을 요청        
				if (type == 1)
					apiSearch = apiSearch + " 크리닝"
				else if (type == 2)
					apiSearch = apiSearch + "코인세탁"

				ps.keywordSearch(apiSearch, placesSearchCB);
			}
		})
	}
	...
	function displayPlaces(places) {
		var listEl = "";
		listEl = document.getElementById('placesList'), menuEl = document
		...
		for (var i = 0; i < places.length; i++) {
			...
			var placePosition = new kakao.maps.LatLng(places[i].y, places[i].x), marker = addMarker(
					placePosition, i), itemEl = getListItem(i, places); // 검색 결과 항목 Element를 생성합니다 
			itemEl = itemel
	...
	function getListItem(i, place) {
		var tf = true;
		for (let t = 0; t < dbData.length; t++) {

			if (place[i].place_name == dbData[t].bname) {

				place[i].place_name = dbData[t].bname

				//주소
				var splitDBAddr = dbData[t].address.split(",")
				var addr = splitDBAddr[1]
				place[i].road_address_name = addr
				place[i].phone = dbData[t].phone
				var contentNum = dbData[t].content
				var grade = dbData[t].grade
				var bno = dbData[t].bno
				var bizMno = dbData[t].bizMno
	...
	function wirteHtml(i, place, contentNum, grade, bno, bizMno) {
		...
		var el = document.createElement('table'), itemStr = '<tbody class="place_body">'
				+ '<tr>'
				+ '<td class="place_name" id="bizMno'+bizMno+'" value='+bno+'>'
				+ (i + 1)
				+ '&nbsp;&nbsp;'
				+ place.place_name
				+ '</td>'
		...
		itemStr += '<tr><td class="place_address">' + place.road_address_name
				+ '</td></tr>'
		if (place.phone != "") {
			itemStr += '<tr><td class="place_phone">' + place.phone
					+ '</td><tr>'
		}
		+'</tbody>';
